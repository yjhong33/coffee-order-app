  ...
  return (
    <div style={{ padding: '54px 0 96px', animation: 'cc-fade .2s ease' }}>
      <div style={{ padding: '0 20px' }}>
        <div style={{ fontSize: 22, fontWeight: 800, letterSpacing: '-.5px' }}>MY</div>
        <div style={{ background: 'var(--cc-card)', border: '1px solid var(--cc-line)', borderRadius: 18, padding: 16, display: 'flex', alignItems: 'center', gap: 14, marginTop: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 16, fontWeight: 700 }}>취합 담당자님</div>
            <div style={{ fontSize: 12, color: 'var(--cc-ink2)', marginTop: 3 }}>익명 세션 · 지금까지 12번 취합했어요</div>
          </div>
        </div>
      </div>

      <div style={{ padding: '22px 20px 0' }}>
        <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>내 취향 설정</div>
        <div style={{ background: 'var(--cc-card)', border: '1px solid var(--cc-line)', borderRadius: 18, padding: '6px 16px' }}>
          {prefs.map((pf, pi) => (
            <div key={pf.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0', borderBottom: pi === prefs.length - 1 ? 'none' : '1px solid var(--cc-line)' }}>
              <span style={{ fontSize: 14, fontWeight: 600 }}>{pf.label}</span>
      ...
      <div style={{ padding: '20px 20px 0' }}>
        <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>최근 그룹 주문</div>
        {recentOrders.map((ro) => (
          <div key={ro.cafeId} onClick={() => onReorder(ro.cafeId)} style={{ background: 'var(--cc-card)', border: '1px solid var(--cc-line)', borderRadius: 15, padding: 13, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', marginBottom: 10 }}>
            <div style={{ width: 44, height: 44, borderRadius: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 800, flex: 'none', background: ro.color, color: ro.fg }}>{ro.initial}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 14, fontWeight: 700 }}>{ro.name}</div>
              <div style={{ fontSize: 12, color: 'var(--cc-ink2)', marginTop: 2 }}>{ro.summary} · {ro.date}</div>
            </div>
            <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--cc-green)', background: 'var(--cc-green-soft)', padding: '6px 11px', borderRadius: 9 }}>재주문</div>
          </div>
        ))}
      </div>
    </div>
